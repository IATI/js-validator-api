...
function groupListIntoMap(list, keyGetter) {
    const result = new Map();
    list.forEach(listItem => {
         const itemKey = keyGetter(listItem);
         const subList = result.get(itemKey);
         if (subList === undefined) {
             result.set(itemKey, [listItem]);
         } else {
...
export default async function validateAdvisories(iatiVersion, iatiXml, adjustLineCountForMissingXmlTag, showDetails = false) {
    ...
    const elementsMatchingAdvisoryGrouped = groupListIntoMap(elementsMatchingAdvisoryXPath, 
                                                             advisory => advisory.advisoryTestName);

    // eslint-disable-next-line no-restricted-syntax
    for (const [advisoryTestName, elementsMatchingAdvisory] of elementsMatchingAdvisoryGrouped) {
